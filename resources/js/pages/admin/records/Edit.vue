<template>
    ...
    <div class="w-full max-w-5xl mx-auto px-4 md:px-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <h1 class="text-2xl md:text-3xl font-semibold text-white mb-4 sm:mb-0">Editar Registro</h1>
            <Link 
        ...
        <form @submit.prevent="submitForm" class="w-full">
            <div class="space-y-6 bg-gray-800/90 p-4 md:p-6 rounded-lg shadow-lg">
                ...
                <div>
                    ...
                    <div v-if="previewRegistro || registro.foto_registro" class="mt-3">
                        <p class="text-sm text-gray-300 mb-2">Vista previa:</p>
                        <img 
                            :src="previewRegistro || getImageUrl(registro.foto_registro)" 
                            alt="Vista previa del registro" 
                            class="max-w-full max-h-48 h-auto rounded"
                        >
                    </div>
                </div>
...
const form = useForm({
    dispositivo_id: props.registro.dispositivo_id,
    hora_entrada: props.registro.hora_entrada,
    hora_salida: props.registro.hora_salida || '',
...
const submitForm = (): void => {
    form.post(route('admin.records.update', props.registro.id), {
        method: 'put',
        forceFormData: true
    });
};
